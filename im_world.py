...
class ImWorldModel:
    ...
    def init_model(self):
        ...
        shared_model.add(Dense(256, kernel_initializer="lecun_uniform", input_shape=(2 * WORLD_W * WORLD_H + Actions.NUM_ACTIONS + Observe.TotalOptions,)))
    ...
    for i in range(sa.num_iter):
        ...
        while(step_count < im.episode_maxlen and not done_flag):
                ...
                if not done_flag:
                    ...
                    while obs_quads and obs_quad < Observe.NUM_QUADRANTS:
                        ...
                        if(random.random() < im.epsilon):
                            ...
                            obs_quad = (np.argmax(qval_obs[obs_quads]))
            ...
            if (len(sa.replay) > 2 * sa.batchsize):
                ...
                sa.act_model.fit(X_act_train, Y_act_train, batch_size=sa.batchsize, epochs=1, verbose=0, callbacks=[tb_act])
            else:
                imworld_train = False
            ...
            if (len(im.replay) > 2 * im.batchsize and imworld_train):
                ...
                sa.obs_model.fit(X_obs_train, Y_obs_train, batch_size=sa.batchsize, epochs=1, verbose=0, callbacks=[tb_obs])
            else:
                imworld_train = False
            ...
            if imworld_train:
                ...
                im.imworld_model.fit(X_imworld_train, Y_imworld_train, batch_size=sa.batchsize, epochs=1, verbose=0, callbacks=[tb_im_model])
                im.reward_model.fit(X_imworld_train, Y_imreward_train, batch_size=sa.batchsize, epochs=1, verbose=0, callbacks=[tb_im_reward])
